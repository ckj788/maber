...
// Generate Compatibility pages
const COMPATIBILITY_DIR = path.join(PUBLIC_DIR, 'compatibility');
if (!fs.existsSync(COMPATIBILITY_DIR)) {
  fs.mkdirSync(COMPATIBILITY_DIR, { recursive: true });
}

const COMPATIBILITY_TRAITS = {
  "1": { name: "The Initiator", theme: "pioneering drive", desc: "independent leader who moves first and decides fast" },
  "2": { name: "The Mediator", theme: "gentle harmony", desc: "cooperative peacemaker who attunes to the room and builds trust" },
  "3": { name: "The Muse", theme: "creative spark", desc: "expressive creator who broadcasts ideas and inspires action" },
  "4": { name: "The Builder", theme: "grounded structure", desc: "orchestrator and founder who turns chaos into systematic order" },
  "5": { name: "The Voyager", theme: "dynamic curiosity", desc: "explorer and persuader who thrives on change and open horizons" },
  "6": { name: "The Healer", theme: "protective nurture", desc: "harmonizer and caregiver who stabilizes groups and nurtures trust" },
  "7": { name: "The Sage", theme: "deep analysis", desc: "analytical seeker who reads the patterns beneath patterns" },
  "8": { name: "The Power", theme: "strategic ambition", desc: "strategist and legacy builder who orchestrates resources" },
  "9": { name: "The Empath", theme: "humanitarian vision", desc: "synthesizer who connects through deep emotional meaning" }
};

// Function to calculate deterministic compatibility scores
function getScores(n1, n2) {
  const i1 = parseInt(n1);
  const i2 = parseInt(n2);
  
  // Define triads: Intellectual (1,5,7), Practical (2,4,8), Emotional/Creative (3,6,9)
  const getGroup = (n) => {
    if ([1, 5, 7].includes(n)) return 'intellectual';
    if ([2, 4, 8].includes(n)) return 'practical';
    return 'emotional';
  };
  
  const g1 = getGroup(i1);
  const g2 = getGroup(i2);
  ...
  if (i1 === i2) {
    ...
    // Cross-triad compatibility
    if ((g1 === 'intellectual' && g2 === 'emotional') || (g1 === 'emotional' && g2 === 'intellectual')) {
      baseLove = 84;
      baseFriend = 86;
      baseCareer = 76;
    } else if ((g1 === 'practical' && g2 === 'intellectual') || (g1 === 'intellectual' && g2 === 'practical')) {
      baseLove = 74;
      baseFriend = 79;
      baseCareer = 89; // Great business match
    } else {
      // Practical + Emotional
      baseLove = 78;
      baseFriend = 81;
  ...
  const variance = (i1 * i2) % 7;
  return {
    love: baseLove + variance,
    friendship: baseFriend + (variance % 5),
    career: baseCareer + (variance % 6)
  };
}

// Function to generate HTML for compatibility pages
function generateCompatibilityHTML(n1, n2) {
  const t1 = COMPATIBILITY_TRAITS[n1];
  const t2 = COMPATIBILITY_TRAITS[n2];
  const scores = getScores(n1, n2);
  
  const pageTitle = `Life Path ${n1} & ${n2} Compatibility: Love, Friendship & Career | OMNIORA`;
  const metaDesc = `Read the detailed cosmic compatibility for Life Path ${n1} (${t1.name}) and Life Path ${n2} (${t2.name}) in romance, work, and social connection.`;
  
  // Write descriptions dynamically
  const loveDesc = `Romantically, the relationship between Life Path ${n1} and Life Path ${n2} is an interplay of ${t1.theme} and ${t2.theme}. As a ${t1.desc}, Life Path ${n1} brings unique strength, while Life Path ${n2} contributes as a ${t2.desc}. Together, they form a relationship score of ${scores.love}% in romantic alignment, which thrives when both partners establish clear boundaries and learn to balance their core frequencies.`;
  
  const friendDesc = `In social circles and friendships, these two numbers share a resonance score of ${scores.friendship}%. This connection represents a unique synergy where ${t1.name} and ${t2.name} learn from each other's lifestyle choices. Communication is usually open, and conflicts are easily softened when they respect each other's individual space.`;
  
  const careerDesc = `For business ventures and professional partnerships, their synergy scores at ${scores.career}%. Life Path ${n1}'s analytical and ${t1.theme} approach complements Life Path ${n2}'s ${t2.theme} execution perfectly, making them a balanced team for legacy building and structural creation.`;
...
<head>
  ...
  <meta name="theme-color" content="#0b0b0c" />
  <link rel="canonical" href="https://omniora13.com/compatibility/life-path-${n1}-and-${n2}" />
  
  <!-- Open Graph / Facebook -->
  <meta property="og:type" content="website" />
  <meta property="og:url" content="https://omniora13.com/compatibility/life-path-${n1}-and-${n2}" />
  <meta property="og:title" content="${pageTitle}" />
  <meta property="og:description" content="${metaDesc}" />
  
  <!-- Twitter -->
  <meta property="twitter:card" content="summary_large_image" />
  <meta property="twitter:url" content="https://omniora13.com/compatibility/life-path-${n1}-and-${n2}" />
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .big-num {
      font-size: 90px; line-height: 1; font-weight: 900; font-family: "Iowan Old Style", "Georgia", serif;
      background: linear-gradient(180deg, #ffffff 30%, rgba(255,255,255,0.1) 100%);
      -webkit-background-clip: text; background-clip: text; color: transparent;
    }
    ...
    /* Scores */
    .scores-container { display: grid; gap: 18px; margin: 24px 0 32px; }
    .score-row { display: flex; align-items: center; gap: 16px; font-size: 13px; font-family: ui-monospace, SFMono-Regular, monospace; }
    .score-label { width: 180px; color: var(--ink-dim); text-transform: uppercase; letter-spacing: 0.08em; }
    .bar-bg { flex: 1; height: 8px; background: rgba(255,255,255,0.04); border-radius: 4px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05); }
    .bar-fill { height: 100%; background: linear-gradient(90deg, rgba(255,255,255,0.08), rgba(255,255,255,0.7)); border-radius: 4px; }
    .score-value { width: 45px; text-align: right; color: var(--ink); font-weight: 600; }
...
<body>
  ...
  <main class="main-wrap">
    <div class="container content-grid">
      <header class="page-head">
        <span class="kicker">Relationship Alignment</span>
        <h1 class="page-title serif">Life Path ${n1} & ${n2} Compatibility</h1>
      </header>

      <section class="card">
        <div class="number-display" style="gap: 24px;">
          <div class="big-num">${n1}</div>
          <div style="font-size: 32px; color: var(--ink-dim); font-family: 'Cinzel', Georgia, serif; line-height: 1;">&</div>
          <div class="big-num">${n2}</div>
        </div>
        
        <h2>Cosmic Resonance Analysis</h2>
        
        <div class="scores-container">
          <div class="score-row">
            <span class="score-label">Love Match</span>
            <div class="bar-bg">
              <div class="bar-fill" style="width: ${scores.love}%"></div>
            </div>
            <span class="score-value">${scores.love}%</span>
          </div>
          <div class="score-row">
            <span class="score-label">Friendship Resonance</span>
            <div class="bar-bg">
              <div class="bar-fill" style="width: ${scores.friendship}%"></div>
            </div>
            <span class="score-value">${scores.friendship}%</span>
          </div>
          <div class="score-row">
            <span class="score-label">Career Synergy</span>
            <div class="bar-bg">
              <div class="bar-fill" style="width: ${scores.career}%"></div>
            </div>
            <span class="score-value">${scores.career}%</span>
          </div>
        </div>

        <div class="section-title">Romantic Connection (Love)</div>
        <div class="text-block">
          <p>${loveDesc}</p>
        </div>

        <div class="section-title">Social Resonance (Friendship)</div>
        <div class="text-block">
          <p>${friendDesc}</p>
        </div>

        <div class="section-title">Professional Flow (Career)</div>
        <div class="text-block">
          <p>${careerDesc}</p>
        </div>
      </section>
      ...
      <section class="cta-card">
        <h2 class="cta-title">Want to calculate your exact relationship blueprint?</h2>
        <p class="cta-desc">
          Life path numbers are only a single coordinate. The OMNIORA calculator maps your exact birth dates, times, and names to build a multi-dimensional connection blueprint.
        </p>
        <div class="btn-group">
          <a class="btn primary" href="/index.html">🏠 Calculate Your Connection for Free</a>
          <a class="btn secondary" href="/index.html#unlock">🔒 Unlock Your Complete Compatibility Report</a>
        </div>
      </section>
    </div>
  </main>
...
for (let i = 1; i <= 9; i++) {
  for (let j = 1; j <= 9; j++) {
    const htmlContent = generateCompatibilityHTML(String(i), String(j));
    const filePath = path.join(COMPATIBILITY_DIR, `life-path-${i}-and-${j}.html`);
    fs.writeFileSync(filePath, htmlContent, 'utf8');
    compatibilityPages.push(`/compatibility/life-path-${i}-and-${j}.html`);
  }
}

const allUrls = [...staticPages, ...lifePathPages, ...compatibilityPages];
...
${allUrls.map(url => `  <url>
    ...
    <priority>${url === '' ? '1.0' : url.startsWith('/life-path') ? '0.8' : url.startsWith('/compatibility') ? '0.7' : '0.5'}</priority>
  </url>`).join('\n')}
</urlset>
`;
